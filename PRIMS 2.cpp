...
//	nodes=nodes-1;
	...
	//to have a array for selected elements
	int selected[nodes];
	for(int i=0;i<nodes;i++){
		selected[i]=false;
	}
		...
		for(int i=0;i<nodes;i++){
			if(selected[i]==true){
				for(int j=0;j<nodes;j++){
					...
					//if the node is not selected and weight is not zero
					if(selected[j]==false && graph[i][j]!=0){
						
						//if the weight is small than min value
						if(min>graph[i][j]){
							min=graph[i][j];
							x=i;
							y=j;
    ...
    auto duration_prims = duration_cast<microseconds>(stop_prims - start_prims);

    cout << "\nTime taken by Prims Algorithm : "<< duration_prims.count() << " microseconds" << endl;
